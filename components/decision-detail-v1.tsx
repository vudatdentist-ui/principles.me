...
export function DecisionDetailV1({ decisionId }: { decisionId: string }) {
  ...
  useEffect(() => {
    refresh().catch((e) =>
      setError(e instanceof Error ? e.message : "Could not load decision.")
    );
  }, [refresh]);
  ...
  async function submitJudgment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await json(`/api/decisions/${decisionId}/judgment`, {
        body: JSON.stringify({
          confidencePercent: Number(form.get("confidence") ?? 65),
      ...
      setError(e instanceof Error ? e.message : "Could not save judgment.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function submitPrinciple(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      await json(`/api/decisions/${decisionId}/principles`, {
        body: JSON.stringify({
          description:
      ...
      setError(e instanceof Error ? e.message : "Could not save principle.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function submitOutcome(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    try {
      await json(`/api/decisions/${decisionId}/outcome`, {
        body: JSON.stringify({
  ...
  if (!detail) {
    return (
      <WorkspaceShell active="decisions" title="Decisions">
        <div className={styles.state}>{error || "Loading…"}</div>
      </WorkspaceShell>
    );
  }
  const evidence = Array.isArray(detail.decision.evidence)
    ? detail.decision.evidence.filter(
  ...
  return (
    <WorkspaceShell active="decisions" title="Decisions">
      <div className={styles.page}>
        <a className={styles.back} href="/decisions">
          <ArrowLeft size={14} /> Decisions
        </a>
        <header className={styles.header}>
          <div>
            <span className={styles.meta}>{detail.decision.status}</span>
            <h1>{detail.decision.question}</h1>
          </div>
          <a className={styles.ask} href="/ask">
            Ask
          </a>
        </header>
        ...
        <section className={styles.section}>
          <h2>Context</h2>
          <p>{detail.decision.context || "—"}</p>
        </section>
        {detail.decision.councilAnalysis || evidence.length ? (
          <section className={styles.section}>
            <h2>Analysis</h2>
            {detail.decision.councilAnalysis ? (
              <p className={styles.pre}>{detail.decision.councilAnalysis}</p>
            ) : null}
            {evidence.length ? (
              <details>
                ...
                <div className={styles.sources}>
                  {evidence.map((item, index) => (
                    <div key={String(item.key ?? index)}>
                      <strong>
                        {String(item.title ?? `Source ${index + 1}`)}
                      </strong>
                      {item.text ? (
                        <p>{String(item.text).slice(0, 700)}</p>
                      ) : null}
                    </div>
                  ))}
                </div>
        ...
        <section className={styles.section}>
          <div className={styles.sectionHead}>
            <h2>Judgment</h2>
            <span>{detail.judgments.length}</span>
          </div>
          {detail.judgments.map((j) => (
            <div className={styles.row} key={j.id}>
              <strong>{j.summary}</strong>
              {j.rationale ? <p>{j.rationale}</p> : null}
              <span>
                {j.confidencePercent === null
                  ? (j.confidence ?? "—")
                  : `${j.confidencePercent}%`}
              </span>
            </div>
        ...
        <section className={styles.section}>
          <div className={styles.sectionHead}>
            <h2>Principles</h2>
            <span>{detail.principles.length}</span>
          </div>
          ...
          {detail.decision.principleCandidate?.status === "pending" ? (
        ...
        <section className={styles.section}>
          <div className={styles.sectionHead}>
            <h2>Outcome</h2>
            <span>{detail.outcomes.length}</span>
          </div>
          {detail.outcomes.map((o) => (
            <div className={styles.row} key={o.id}>
              <strong>{o.result}</strong>
              {o.lessons ? <p>{o.lessons}</p> : null}
              <span>{o.verdict.replace("_", " ")}</span>
            </div>
          ))}
          <form className={styles.form} onSubmit={submitOutcome}>
            <textarea name="result" placeholder="What happened?" required />
            <textarea name="lessons" placeholder="What did you learn?" />
