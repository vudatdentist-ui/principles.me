...
for (const filePath of files) {
  // biome-ignore lint/performance/noAwaitInLoops: Upload sequentially to avoid overloading the RAGFlow ingestion endpoint.
  ...
  console.log(
    JSON.stringify(
      { file: path.basename(filePath), response: payload },
      null,
      2
    )
  );
