...
test("authenticated core surfaces default to Vietnamese", async ({ page }, testInfo) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Mục tiêu mới", level: 1, exact: true }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Tri thức", level: 1, exact: true }),
  ).toBeVisible();

  await page.getByRole("link", { name: "Học hỏi", exact: true }).click();
  await expect(
    page.getByRole("heading", { name: "Học hỏi", level: 1, exact: true }),
  ).toBeVisible();

  await page.getByRole("link", { name: "Tổ chức", exact: true }).click();
  await expect(
    page.getByRole("heading", { name: "Tổ chức", level: 1, exact: true }),
  ).toBeVisible();
