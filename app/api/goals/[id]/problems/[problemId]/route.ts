...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid problem command." }, { status: 400 });
  }
  ...
  if (command.command === "link_principle") {
    ...
    return result
      ...
      : NextResponse.json({ error: "Problem or principle not found." }, { status: 404 });
  }

  if (command.command === "adopt_candidate") {
    const result = await adoptProblemCandidate({ goalId: id, problemId, userId });
    return result
      ? NextResponse.json({ principle: result }, { status: 201 })
      : NextResponse.json({ error: "No candidate principle." }, { status: 409 });
  }

  const detail = await getGoalDetail(id, userId);
  const selectedProblem = detail?.problems.find((item) => item.id === problemId);
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Cortex is not configured for Goals." },
      { status: 503 }
    );
  }
}
