...
import { createVerificationTokenForEmail } from "@/features/auth/repository";
import {
  authRateScope,
  consumeRateLimit,
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const pending = await createVerificationTokenForEmail(parsed.data.email);
    if (pending) {
      await sendVerificationEmail(pending.email, pending.token);
    }
    ...
    console.error(JSON.stringify({ event: "auth_verification_resend_failed" }));
    return Response.json(
