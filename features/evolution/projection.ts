...
function nextAction(
  ...
  if (stage === "dream") {
    return {
      kind: "clarify_dream",
      label: "Clarify your dream",
      prompt: "What do you really want?",
    };
  }
  if (stage === "reality") {
    return {
      kind: "observe_reality",
      label: "Face reality",
      prompt: "What is actually true right now?",
    };
  }
  if (stage === "problem") {
    return {
      kind: "identify_problem",
      label: "Name the problem",
      prompt: "Where does reality fall short of the dream?",
    };
  }
  if (stage === "diagnosis") {
    return {
      kind: "diagnose_problem",
      label: "Diagnose the root cause",
      prompt: "Why is this happening?",
    };
  }
  if (stage === "design") {
    return {
      kind: "design_change",
      label: "Design the machine",
      prompt: "What must change so this problem stops recurring?",
    };
  }
  if (stage === "do") {
    return {
      kind: "do_design",
      label: "Do the design",
      prompt: "What commitment moves this machine change forward now?",
    };
  }
  if (stage === "outcome") {
    return {
      ...
      prompt: "What actually happened after you changed the machine?",
    };
  }
  if (stage === "reflection") {
    return {
      kind: "reflect_on_pain",
      label: "Reflect",
      prompt: "What hurt or surprised you, and what is it teaching you?",
    };
  }
  if (!principle) {
    return {
      kind: "distill_principle",
      label: "Distill a principle",
      prompt: "What rule is worth testing next time?",
    };
  }
  if (principle.acceptanceState === "pending") {
    return {
      kind: "review_principle",
      label: "Review the principle",
      prompt: "Is this a rule you are willing to test against reality?",
    };
  }
  return {
    kind: "continue_cycle",
    label: "Return to reality",
    prompt: "What is true now after applying what you learned?",
  };
}
...
function attention(input: {
  ...
  if (input.outcome && !input.outcomeReview) {
    items.push({
      kind: "pain_needs_reflection",
      title: "This outcome needs reflection.",
    });
  }
  if (input.principle?.acceptanceState === "pending") {
    items.push({
      kind: "principle_needs_review",
      title: "A Principle candidate needs your judgment.",
    });
  } else if (
    input.principle &&
    (input.principle.acceptanceState === "accepted" ||
      input.principle.acceptanceState === "revised") &&
    input.principle.lifecycleState !== "retired"
  ) {
    items.push({
      kind: "principle_under_test",
      title: "A Principle is under test against reality.",
    });
  }
  return items;
}
...
export function projectEvolutionState(
  ...
  const dream =
    people.goals.find((goal) => goal.status === "chosen") ?? people.goals[0] ?? null;
  const reality = dream
    ? people.reality.find((item) => item.goalId === dream.id) ?? null
    : null;
  const problem = dream
    ? people.problems.find(
        (item) => item.goalId === dream.id && item.status === "recognized"
      ) ?? people.problems.find((item) => item.goalId === dream.id) ?? null
    : null;
  ...
  const principle = reflection
    ? people.principles.find(
      ...
      people.principles.find((item) => item.originReflectionId === reflection.id) ??
      null
    : null;
